...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/submit_housebuild_collm_cc_slurm_job.sh [config.env] [KEY=VALUE ...]

Examples:
  bash scripts/submit_housebuild_collm_cc_slurm_job.sh DRY_RUN=true RUN_SUFFIX=dryrun
  bash scripts/submit_housebuild_collm_cc_slurm_job.sh configs/cluster/house_build_collm_cc_3gpu.env DRY_RUN=true RUN_SUFFIX=dryrun
  bash scripts/submit_housebuild_collm_cc_slurm_job.sh NUM_EPOCHS=1 TRAIN_SPLIT='[:1]' EVAL_SPLIT=None MAX_NEW_TOKENS=64 RUN_SUFFIX=smoke

This wrapper submits slurm/house_build_collm_cc_3gpu.sbatch with explicit
sbatch flags. CoLLM-CC uses 2 decentralized actor LLMs plus 1 centralized
critic LLM, so the default request is 3 GPUs on one node.
EOF
}
...
export SLURM_JOB_NAME="${SLURM_JOB_NAME:-hb-collm-cc-3gpu}"
...
export SLURM_GPUS_PER_NODE="${SLURM_GPUS_PER_NODE:-3}"
export SLURM_NTASKS_PER_NODE="${SLURM_NTASKS_PER_NODE:-1}"
export SLURM_CPUS_PER_TASK="${SLURM_CPUS_PER_TASK:-40}"
...
echo "sbatch ${SBATCH_LAUNCH_ARGS} $PROJECT_ROOT/slurm/house_build_collm_cc_3gpu.sbatch"

if [[ "${SUBMIT_DRY_RUN:-0}" == "1" ]]; then
  exit 0
fi

sbatch "${SBATCH_ARGS[@]}" "$PROJECT_ROOT/slurm/house_build_collm_cc_3gpu.sbatch"
