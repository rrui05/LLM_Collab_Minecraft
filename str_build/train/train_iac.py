...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train StrBuild with IAC (CoMLRL IACTrainer)")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(REPO_ROOT, "str_build", "configs", "str_build_iac_config.yaml"),
    ...
    if not isinstance(iac_cfg, dict):
        iac_cfg = {}
    seed_value = int(cfg.get("seed", iac_cfg.get("seed", 42)))
