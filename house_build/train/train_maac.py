...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train HouseBuild with MAAC (CoMLRL MAACTrainer)")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(REPO_ROOT, "house_build", "configs", "house_build_maac_config.yaml"),
    ...
    if num_agents < 1:
        raise ValueError("maac.num_agents must be >= 1")
